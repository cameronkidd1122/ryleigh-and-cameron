...
(() => {
  ...
  const creatures = ['black', 'tortie', 'dog'].map((kind, index) => {
    ...
    const pet = {element, canvas, context, artwork, ready:false, index, kind, x:0, y:0, size:0, gait:0, frame:2, next:2, mix:0, walking:false, quiet:0};
    artwork.onload = () => {
  ...
  function setFrame(pet, frame) {
    pet.frame = pet.next = frame;
    pet.mix = 0;
    renderFrame(pet);
  }
  function renderFrame(pet) {
    pet.canvas.dataset.frame = String(pet.frame);
    pet.canvas.dataset.mix = pet.mix.toFixed(4);
    if (!pet.ready || !pet.context) return;
    const ctx = pet.context;
    ctx.clearRect(0,0,96,96);
    ctx.imageSmoothingEnabled = false;
    // Exact source rectangles never slide across other cells. Additive blending
    // preserves opaque bodies while the small leg changes ease between poses.
    ctx.globalCompositeOperation = 'source-over';
    ctx.globalAlpha = 1-pet.mix;
    ctx.drawImage(pet.artwork,(pet.frame%4)*96,Math.floor(pet.frame/4)*96,96,96,0,0,96,96);
    if (pet.mix > 0) {
      ctx.globalCompositeOperation = 'lighter';
      ctx.globalAlpha = pet.mix;
      ctx.drawImage(pet.artwork,(pet.next%4)*96,Math.floor(pet.next/4)*96,96,96,0,0,96,96);
    }
    ctx.globalAlpha = 1;
    ctx.globalCompositeOperation = 'source-over';
  }
  function blendGait(pet) {
    ...
    pet.frame=frame;pet.next=next;pet.mix=mix;
    renderFrame(pet);
  }
  function paint(pet) {

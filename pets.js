...
(() => {
  ...
  let enabled = true;
  try { enabled = localStorage.getItem('wedding-pets-visible') !== 'false'; } catch {}
  ...
  const creatures = ['black', 'tortie', 'dog'].map((kind, index) => {
    ...
    element.style.backgroundImage = `url("${new URL('pet-' + kind + '.png', assetBase)}")`;
    element.dataset.pet = kind;
    layer.append(element);
    return {element, index, kind, x:0, y:0, size:0, gait:0, frame:-1};
  });
  ...
  function setFrame(pet, frame) {
    ...
    pet.frame = frame;
    pet.element.style.backgroundPosition = `${frame % 2 ? 100 : 0}% ${frame > 1 ? 100 : 0}%`;
  }
  function paint(pet) {
  ...
  function park() {
    ...
    for (const pet of creatures) {
      ...
      setFrame(pet,1); paint(pet); x += pet.size+4;
    }
  }
  function measure() {
  ...
  function collectObstacles(now) {
    ...
    obstacles.push(toggle.getBoundingClientRect());
  }
  ...
  function tick(now) {
    ...
    creatures.forEach((pet,index)=>{
      ...
      if(distance>1.5){pet.x+=dx/distance*step;pet.y+=dy/distance*step;pet.gait+=step;setFrame(pet,Math.floor(pet.gait/7)%4);}
      else setFrame(pet,1);
  ...
  toggle.addEventListener('click',()=>{
    enabled=!enabled;
    try {localStorage.setItem('wedding-pets-visible',String(enabled));}catch{}
    start();
  });
